...
revenue_by_month_tbl <- bike_orderlines_tbl %>%
    ...
    mutate(year_month = floor_date(x= order_date,unit="months")%>%ymd()) %>%
    group_by(year_month) %>%
...
revenue_by_category_2 %>%
    mutate(category_2 = as_factor(category_2)%>%fct_reorder(revenue))%>%
...
bike_orderlines_tbl %>%
    ...
    geom_histogram() +
    facet_wrap(vars(frame_material), ncol = 1) +
    scale_fill_tq()+
    theme_tq()
