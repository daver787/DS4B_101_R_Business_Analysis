...
RColorBrewer::brewer.pal(n=100, name="Blues")[1]%>%col2rgb()
# Viridis
...
sales_by_year_category_2_tbl %>%
    ...
    facet_wrap(vars(category_2),nrow=3,ncol =3,scales ="free_y")+
    expand_limits(y= 0)
...
# - Categorical (e.g. Category 2): Changes color via discrete palette
...
sales_by_year_category_2_tbl %>%
    ...
    labs(
        ...
        y        = "Revenue in ($M)",
        fill     = "2nd Category",
        caption  = "Bike sales look strong heading into 2016" 
    ) +
    theme_light() +
    theme(
