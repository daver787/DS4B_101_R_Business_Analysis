...
#Step 3 : Use a mutate() +map()
excel_tbl_nested_fixed <- excel_tbl_nested %>%
    mutate(data_fixed = data %>% map(select_non_na))


excel_tbl_nested_fixed %>% select(-data) %>%ungroup()
...
fit_loess_cross_country_model <- sales_by_m_cross_country_tbl %>%
...
tidy_loess <- function(data, span =0.2){
    ...
    fit_loess <- loess(total_price ~ month_end_num,
                       data = data_formatted,
                       span = span)
