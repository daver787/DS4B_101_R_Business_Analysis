...
rolling_avg_3_tbl <- bike_orderlines_tbl %>%
    ...
    mutate(month_end = ceiling_date(order_date, "month") - period(1,unit = "day")) %>%
...
c("separate_bike_model","detect_outliers") %>%
    dump(file = "00_scripts/separate_bikes_and_outlier_detection.R", append = TRUE)
