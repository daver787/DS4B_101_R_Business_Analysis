...
function(interactive = TRUE) {
    ...
    pct_sales_by_customer_tbl <- bike_orderlines_tbl %>%
        ...
        mutate(label_text = str_glue("Customer: {bikeshop_name}
                                      ...
                                      Percent of Sales: {scales::percent(pct, accuracy =.1)}"))
    
    # VISUALIZATION
   g <-  pct_sales_by_customer_tbl %>%
        ggplot(aes(category_2, bikeshop_name)) +
        geom_tile(aes(fill = pct ) ) +
        geom_text(aes(label = scales::percent(pct,accuracy =  .1), text = label_text), size = 3) +
...
function(top_n_products = 10, 
    ...
    top_n_tbl <- bike_orderlines_tbl %>%
        ...
        ungroup() %>%
        left_join(combined_tbl%>%select(bikeshop_name, .cluster), by ="bikeshop_name") %>%
   ...
   g <-  top_n_tbl %>%
        ...
        theme(
            #legend.position = "none",
            strip.text.x = element_text(margin = margin(5, 5, 5, 5, "pt"))) +
        scale_color_tq() +
