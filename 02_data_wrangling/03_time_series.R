...
i/ ddays(1) #interval / days = how many days in the interval
i / dminutes(1) #interval / minutes = how many minutes in the interval
#always use a duration when dividing an interval
order_date_tbl %>%
    mutate(today = today()) %>%
...
bike_sales_y_tbl %>%
    ...
    mutate(pct_diff_1 = scales::percent(pct_diff))
...
bike_sales_y_tbl %>%
    mutate(cummulative_sales = cumsum(sales),
           cummulative_sales_avg = cummean(sales))
...
bike_sales_y_tbl %>%
    ...
    mutate(cummulative_sales_pct_chr = scales::percent(cummulative_sales_pct))
...
bike_sales_m_tbl %>%
    mutate(roll_mean_3 = rollmean(sales, k = 3,na.pad = TRUE,align="right",fill = NA)) %>%#right align is 3 previous values
