...
# Case
bikeshop_name <- c("Ithaca Mountain Climbers") 
...
# Vector
c("Road - Elite Road - Carbon","Road - Elite Road") %>% str_split(" - ",simplify= TRUE)
...
bikes_tbl %>%
  select(model)%>%
  mutate(model_num_removed = model %>% str_replace_all(pattern = "[0-9]",replacement="")%>%str_trim())
...
bike_orderlines_tbl %>%
  set_names(names(.) %>% str_replace(pattern = "_",replacement = ".") %>% str_to_upper())
...
bike_orderlines_colnames_tbl %>%
  select(contains("prod"),total_price)
